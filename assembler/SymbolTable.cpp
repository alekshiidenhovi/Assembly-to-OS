...
SymbolTable::SymbolTable(
    std::initializer_list<std::pair<Symbol, short>> symbol_pairs) {
  for (const auto& [symbol, address] : symbol_pairs) {
    symbol_table_.emplace(symbol, address);
  }
}

void SymbolTable::addEntry(const Symbol& symbol, short address) {
  symbol_table_.emplace(symbol, address);
}
...
short SymbolTable::getAddress(const Symbol& symbol) const {
